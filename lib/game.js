...
class Game {
  ...
  winner() {
    const sign = this._rule.winner(this.board);
    if (sign) this._winner = sign === this.playerOne.sign ? this.playerOne : this.playerTwo;
    return this._winner;
  }

  insertMove(row, column) {
    if (this._winner || this._board.isFull()) throw new Error('The game is over');
    if (this.board[row][column]) throw new Error('Space filled');

    this.board[row][column] = this.selectPlayer().sign;
    if (this.winner()) {
